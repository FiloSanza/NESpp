...
namespace nespp::tests {
    ...
    TEST(TransferOpcodeHandlers, TAX) {
        ...
        EXPECT_EQ(cpu.get_a().get_value(), cpu.get_x().get_value());
    ...
    TEST(TransferOpcodeHandlers, TAY) {
        ...
        EXPECT_EQ(cpu.get_a().get_value(), cpu.get_y().get_value());
        EXPECT_EQ(cpu.get_program_status().get_value(), flags);
    }
    ...
    TEST(TransferOpcodeHandlers, TSX) {
        ...
        cpu.get_x().set_value(x_value);
        cpu.get_stack_pointer().set_value(sp_value);
        cpu.get_program_status().set_value(0);

        machine.execute();

        EXPECT_EQ(cpu.get_stack_pointer().get_value(), cpu.get_x().get_value());
        EXPECT_EQ(cpu.get_program_status().get_value(), flags);
    }
